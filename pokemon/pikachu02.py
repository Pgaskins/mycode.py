...
def main():
    ...
    for item in items.get("results"):
        ...
        if 'ball' in item.get("name"):
            # if TRUE, add that item to the end of list healwords
            ball.append(item.get("name"))

    ## list all
    print(f"There are {len(ball)} words that contain the word 'ball' in the Pokemon Item API!")
    print("List of Pokemon items containing ball: ")
    print(ball)
